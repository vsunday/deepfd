...
BUCKET_NAME = 'rekog2-u'

session = Session();
rekog = session.create_client('rekognition', region_name=REGION_NAME)
s3 = session.create_client('s3', region_name=REGION_NAME)
...
# limit threads? 
class RekogThread(Thread):
    def __init__(self, img_name, raw_img, client, topic):
        ...
        self.topic = topic
    def run(self):
        flg, who = face_compare(self.img_name, self.raw_img)
        if flg:
            msg = 'detected: ' + who
            self.client.publish(topic=self.topic, payload=msg)
        
def face_compare(img_name, raw_img):
    # img file resize
    encode_param=[int(cv2.IMWRITE_JPEG_QUALITY),90]
    _, jpg_data = cv2.imencode('.jpg', raw_img, encode_param)
    ...
    faceDetected = (response['FaceMatches'] != []) and (response['FaceMatches'] != None)
    if faceDetected:
        return True, img_name.split('.')[0]
    else:
        return False, None
...
names = list_s3(BUCKET_NAME) # list of file names
    ...
    try:
        ...
        while True:
            ...
            for obj in parsed_inference_results[model_type]:
                # array result of doInference
                # each element of array corresponds to detected faces
                # iterates all detected faces
                if obj['prob'] > detection_threshold:
                    ...
                    if time.time() - previous_time > 5.0:
                        for name in names:
                            t = RekogThread(name, frame, client, iot_topic)
                            t.start()
                        previous_time = time.time()
